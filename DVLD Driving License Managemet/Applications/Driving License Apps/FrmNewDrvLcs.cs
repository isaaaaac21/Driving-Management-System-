...
namespace DVLD_Driving_License_Managemet.Applications
{
    public partial class FrmNewDrvLcs : Form
    {
        ...
        private bool _PersonIsOlderThanClassMinAge()
        {
            clsLicenseClass ls = clsLicenseClass.GetLicenseClassByID(_CurrentApplication.LicenseClassID);
            if (ls._MinAge > (DateTime.Now.Year - ctrlFilter1.per.DateOfBirth.Year))
            {
                clsDesign._ShowErrorMessage("The person is younger for this type of license\nPlease try again after reaching the minimum age !!!");
                return false; 
            }
            return true; 
        }
